...
class SudokuUI:
    ...
    def _preprocess_cell(self, cell_img):
        ...
        # 形态学操作 - 开运算去除小噪点
        kernel = np.ones((3, 3), np.uint8)  # 增大内核，增强数字轮廓
        processed = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, kernel)
    ...
    def _recognize_digit(self, digit_img):
        """使用Tesseract识别单个数字"""
        # 配置Tesseract
        config = '--oem 3 --psm 10 --dpi 300 -c tessedit_char_whitelist=123456789 -c classifier_min_confidence=50 -c tessedit_enable_doc_dict=0 -c load_system_dawg=0 -c load_freq_dawg=0 -c tessedit_do_invert=0'
        ...
        if not text or not text.isdigit():
            ...
            if contours:
                ...
                # 根据特征识别数字，先识别特征明显的数字
                if aspect_ratio > 1.2 and 0.3 < extent < 0.5:
                    return 7  # 7通常细长且面积中等
                # 更严格的1识别条件：宽高比>1.5、面积小且圆形度低
                elif aspect_ratio > 1.6 and extent < 0.35 and cnt_area < 60 and circularity < 0.25:
                    return 1  # 1通常是细长且面积小的
                elif 0.9 < aspect_ratio < 1.1 and extent > 0.65 and circularity > 0.7:
                    return 8  # 8通常是圆形且面积大
                elif 0.7 < aspect_ratio < 0.9 and 0.4 < extent < 0.6:
                    return 6  # 6通常宽高比小于1
                elif 0.7 < aspect_ratio < 0.9 and extent > 0.6 and circularity > 0.55:
                    return 9  # 9通常宽高比小于1且面积较大
                elif circularity > 0.6 and extent > 0.6:
                    return 0  # 0通常较圆且面积较大
                elif 0.8 < aspect_ratio < 1.0 and 0.35 < extent < 0.5 and circularity < 0.45:
                    return 2  # 2通常有较低的圆形度
                elif 0.85 < aspect_ratio < 1.05 and 0.55 < extent < 0.75 and circularity > 0.5:
                    return 3  # 3通常有中等圆形度
                elif aspect_ratio > 1.1 and 0.45 < extent < 0.65:
                    return 4  # 4通常较宽且面积中等
                elif 0.75 < aspect_ratio < 0.95 and 0.45 < extent < 0.65 and circularity < 0.55:
                    return 5  # 5通常宽高比小于1且圆形度中等
            return 0
