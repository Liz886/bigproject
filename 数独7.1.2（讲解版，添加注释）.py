...
class SudokuUI:
    def __init__(self, root):
        ...
        self.last_board_state = None  # 记录上一次求解的棋盘状态
        self.image_recognized_board = None  # 记录图片识别的棋盘状态
    ...
    def solve_sudoku(self):
        ...
        if conflicts:
            ...
            messagebox.showerror("错误", f"发现{len(conflicts)}个冲突，请修正后再求解！")
            return
        ...
        if solve_result:
            ...
            self.solution = solver.board  # 保存解
            self.last_board_state = copy.deepcopy(board)  # 保存当前状态
        else:
            messagebox.showerror("错误", "求解失败！")
    
    def find_conflicts(self, board):
        """检查整个数独板是否有冲突，返回冲突单元格列表"""
        conflicts = set()
    ...
    def clear_board(self):
        ...
        self.image_recognized_board = None
    ...
    def show_hint(self):
        ...
        # 检查棋盘是否变化
        if self.last_board_state is None or board != self.last_board_state:
            # 需要重新求解
            conflicts = self.find_conflicts(board)
            if conflicts:
                # 标记冲突并提示
                for i, j in conflicts:
                    self.mark_cell_as_conflict(i, j)
                messagebox.showwarning("提示", "当前棋盘存在冲突，请修正后再使用提示功能")
                return
            ...
            if solution_count == 0:
                messagebox.showerror("提示", "该数独无解，无法提示")
                return
            elif solution_count > 1:
                messagebox.showwarning("提示", "该数独存在多个解，无法提示")
                return
            else:
                # 求解得到唯一解
                if solver.solve():
                    ...
                    messagebox.showerror("提示", "求解失败，无法提示")
                    return
        
        # 如果没有保存解
        if self.solution is None:
            messagebox.showerror("提示", "未找到有效解，请先求解数独")
            return
    ...
    def find_sudoku_contour(self, edged):
        """在边缘检测图像中寻找数独轮廓"""
    ...
    def extract_cells_from_grid(self, grid, grid_size=9):
        """从数独网格中提取单元格"""
    ...
    def recognize_sudoku(self, file_path):
        """使用新方法识别数独图片"""
        try:
            # 显示加载窗口
            self.show_loading(title="图片识别中", message="正在加载图片...", show_progress=True)
            self.update_progress(10, "正在加载图片...")
            ...
            try:
                ...
                raise Exception(f"读取图片失败: {str(e)}, 路径: {file_path}")
            
            if img is None:
                raise Exception(f"无法解析图片文件，可能格式不支持: {file_path}")
            
            self.update_progress(20, "正在预处理图像...")
            # 预处理图像
            preprocessed = self.preprocess_image(img)
            
            self.update_progress(30, "正在检测数独网格...")
            # 寻找数独轮廓
            contour = self.find_sudoku_contour(preprocessed)
            if contour is None:
                raise Exception("未检测到数独网格，请确保图片中包含清晰的数独且背景简单")
            
            self.update_progress(40, "正在校正图像...")
            # 进行透视变换
            sudoku_grid = self.perspective_transform(img, contour)
            
            # 转换为灰度图
            sudoku_gray = cv2.cvtColor(sudoku_grid, cv2.COLOR_BGR2GRAY)
            
            self.update_progress(50, "正在提取数独单元格...")
            # 提取单元格
            cells = self.extract_cells_from_grid(sudoku_gray)
            ...
            # 计算每个单元格的进度增量
            progress_per_cell = 45 / (grid_size * grid_size)
            ...
            for i in range(grid_size * grid_size):
                ...
                # 更新进度
                current_progress = 50 + (i + 1) * progress_per_cell
                self.update_progress(current_progress, f"识别单元格 ({row+1}, {col+1})...")
                ...
                cell_cropped = cell[margin:h-margin, margin:w-margin]
                cell_resized = cv2.resize(cell_cropped, (50, 50))
                _, cell_bin = cv2.threshold(cell_resized, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
                kernel = np.ones((3, 3), np.uint8)
                cell_bin = cv2.dilate(cell_bin, kernel, iterations=1)
                cell_bin = cv2.resize(cell_bin, (80, 80))
                text = pytesseract.image_to_string(
                    cell_bin,
                    config='--psm 10 --oem 3 -c tessedit_char_whitelist=123456789lIoOSsBb'
                ).strip()
                text = text.replace('l', '1').replace('I', '1').replace('o', '0').replace('O', '0')
                text = text.replace('S', '5').replace('s', '5').replace('B', '8').replace('b', '6')
                if text == '' or not text.isdigit():
                    # 可选：再试psm 13
                    text2 = pytesseract.image_to_string(
                        cell_bin,
                        config='--psm 13 --oem 3 -c tessedit_char_whitelist=123456789lIoOSsBb'
                    ).strip()
                    text2 = text2.replace('l', '1').replace('I', '1').replace('o', '0').replace('O', '0')
                    text2 = text2.replace('S', '5').replace('s', '5').replace('B', '8').replace('b', '6')
                    if text2.isdigit():
                        text = text2
                if text == '' or not text.isdigit():
                    board[row][col] = 0
            ...
            # 保存识别出的数独状态
            self.image_recognized_board = copy.deepcopy(board)
            
            # 尝试求解识别出的数独
            self.update_progress(95, "正在求解数独...")
            solver = SudokuSolver(copy.deepcopy(board))
            if solver.solve():
                self.solution = solver.board
            else:
                # 如果求解失败，我们仍然设置board到界面，但是solution设为None
                self.solution = None
                self.root.after(0, lambda: messagebox.showwarning("警告", "识别出的数独无解，提示功能将不可用"))
            ...
            self.root.after(0, lambda: setattr(self, 'last_board_state', copy.deepcopy(board)))
            if self.solution is not None:
                self.root.after(0, lambda: messagebox.showinfo("识别成功", "数独图片识别完成！"))
            else:
                self.root.after(0, lambda: messagebox.showinfo("识别完成", "数独图片识别完成，但该数独无解"))
        except Exception as e:
            error_msg = str(e)
