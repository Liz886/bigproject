...
class SudokuUI:
    def __init__(self, root):
        ...
        self.root.geometry("580x700")  # 增加窗口大小
    ...
    def configure_styles(self):
        ...
        # 初始数字样式
        self.style.configure("Initial.TEntry", 
                            font=("Arial", 18, "bold"),
                            foreground="#1976d2")  # 蓝色表示初始数字
        
        # 求解结果样式
        self.style.configure("Solution.TEntry", 
                            font=("Arial", 18),
                            foreground="#2e7d32")  # 绿色表示求解结果
    ...
    def create_widgets(self):
        ...
        # 标题
        title_label = ttk.Label(main_frame, text="数独求解器", style="Title.TLabel")
        title_label.pack(pady=(0, 10))
        
        # 创建数独网格框架
        board_frame = ttk.Frame(main_frame, style="Board.TFrame")
        ...
        for i in range(9):
            ...
            for j in range(9):
                ...
                # 增加单元格间距
                padx = (3, 1) if j % 3 == 0 else (1, 1)
                pady = (3, 1) if i % 3 == 0 else (1, 1)
        ...
        # 功能按钮框架
        btn_frame = ttk.Frame(main_frame, style="ButtonFrame.TFrame")
        btn_frame.pack(fill=tk.X, pady=15)
        
        # 随机数独生成区域
        generate_frame = ttk.Frame(btn_frame)
        generate_frame.pack(side=tk.LEFT, padx=5, fill=tk.X, expand=True)
        
        ttk.Label(generate_frame, text="难度:", font=("微软雅黑", 10)).pack(side=tk.LEFT, padx=(0, 5))
        ...
        generate_btn = ttk.Button(generate_frame, text="生成随机数独", 
                                 command=self.generate_random, style="Generate.TButton")
        ...
        # 求解按钮
        self.solve_btn = ttk.Button(button_frame, text="求解", 
                                   command=self.solve, style="Solve.TButton")
    ...
    def bind_events(self):
        ...
        for i in range(9):
            for j in range(9):
                ...
                self.cells[i][j].bind("<FocusOut>", lambda e, i=i, j=j: self.on_focus_out(e, i, j))
    ...
    def on_focus(self, event, i, j):
        ...
        # 高亮当前单元格和关联单元格
        event.widget.state(['highlight'])
        
        # 高亮同一行
        for col in range(9):
            self.cells[i][col].state(['highlight'])
        
        # 高亮同一列
        for row in range(9):
            self.cells[row][j].state(['highlight'])
        
        # 高亮同一九宫格
        start_row, start_col = i - i % 3, j - j % 3
        for r in range(3):
            for c in range(3):
                self.cells[start_row + r][start_col + c].state(['highlight'])
    ...
    def solve(self):
        ...
        try:
            ...
            messagebox.showerror("求解失败", str(e), parent=self.root)
            self.status_var.set("求解失败! | 难度: -")
        except Exception as e:
            messagebox.showerror("错误", f"发生未知错误: {str(e)}", parent=self.root)
            ...
            for j in range(9):
                ...
                if value != 0:
                    ...
                    if (i, j) in self.initial_cells:
                        self.cells[i][j].configure(style="Initial.TEntry")
                    else:
                        self.cells[i][j].configure(style="Solution.TEntry")
            ...
            for i in range(9):
                for j in range(9):
                    ...
                    if value != 0:
                        ...
                        self.cells[i][j].insert(0, str(value))
                        # 设置生成的数字为初始样式
                        self.cells[i][j].configure(style="Initial.TEntry", state="readonly")
                        # 记录初始数字位置
                        self.initial_cells.add((i, j))
                    else:
                        # 设置空白单元格为可修改
                        self.cells[i][j].configure(style="Sudoku.TEntry", state="normal")
            
            # 应用九宫格交替背景
            for i in range(9):
                for j in range(9):
                    if (i // 3 + j // 3) % 2 == 0:
                        self.cells[i][j].configure(style="SudokuAlt.TEntry")
            
            self.status_var.set(f"已生成{difficulty}难度数独 | 请开始解题")
        except Exception as e:
            messagebox.showerror("错误", f"生成数独失败: {str(e)}", parent=self.root)
    ...
    def clear(self):
        ...
        for i in range(9):
            for j in range(9):
                self.cells[i][j].configure(style="Sudoku.TEntry", state="normal", foreground="black")
                self.cells[i][j].delete(0, tk.END)
