...
class SudokuUI:
    ...
    def recognize_sudoku(self, image_path):
        ...
        try:
            # 清空当前数独
            self.root.after(0, self.clear_board)
            ...
            if sudoku_contour is None:
                ...
                if lines is not None and len(lines) > 5:
                    ...
                    if len(horizontal_lines) >= 10 and len(vertical_lines) >= 10:
                        ...
                        top_left = line_intersection(h1, np.pi/2, v1, 0)
                        top_right = line_intersection(h1, np.pi/2, v2, 0)
                        bottom_left = line_intersection(h2, np.pi/2, v1, 0)
                        bottom_right = line_intersection(h2, np.pi/2, v2, 0)
                        
                        sudoku_contour = np.array([top_left, top_right, bottom_right, bottom_left], dtype=np.float32)
                    else:
                        raise Exception("未检测到足够的网格线")
                else:
                    raise Exception("未检测到数独网格")
            ...
            remove_horizontal = cv2.morphologyEx(warped_thresh, cv2.MORPH_OPEN, horizontal_kernel, iterations=2)
            cnts = cv2.findContours(remove_horizontal, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[0]
            for c in cnts:
                cv2.drawContours(warped, [c], -1, (255,255,255), 2)
            # 检测垂直线
            vertical_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, cell_size//2))
            remove_vertical = cv2.morphologyEx(warped_thresh, cv2.MORPH_OPEN, vertical_kernel, iterations=2)
            cnts = cv2.findContours(remove_vertical, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[0]
            for c in cnts:
                cv2.drawContours(warped, [c], -1, (255,255,255), 2)
            
            for i in range(9):
                for j in range(9):
                    # 提取单元格区域，增加边界偏移以避免网格线干扰
                    offset = max(2, cell_size // 10)
                    x = j * cell_size + offset
                    y = i * cell_size + offset
                    cell = warped[y:y+cell_size-2*offset, x:x+cell_size-2*offset]
                    
                    # 如果单元格过小，跳过
                    if cell.shape[0] < cell_size * 0.5 or cell.shape[1] < cell_size * 0.5:
                        continue
    ...
    def _preprocess_cell(self, cell):
        ...
        # 降低面积阈值以识别更小的数字
        if cv2.contourArea(max_contour) < 30:
            return None
    ...
    def _recognize_digit(self, digit_img):
        ...
        try:
            ...
            # 再次阈值处理确保二值化
            _, digit_img = cv2.threshold(digit_img, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
            
            # 使用更合适的识别参数
            # 使用LSTM引擎和严格的数字识别配置
            config = '--oem 1 --psm 10 -c tessedit_char_whitelist=123456789 -c tessedit_min_confidence=60'
            
            # 识别数字并获取置信度
            data = pytesseract.image_to_data(digit_img, config=config, output_type=pytesseract.Output.DICT)
            text = data['text'][0].strip() if len(data['text']) > 0 else ''
            confidence = float(data['conf'][0]) if len(data['conf']) > 0 else 0
            
            # 只有高置信度的结果才被接受
            if text and text.isdigit() and confidence > 60:
                return int(text)
            return 0
