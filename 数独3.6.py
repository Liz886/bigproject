...
class SudokuUI:
    ...
    def create_widgets(self):
        ...
        for i in range(9):
            ...
            for j in range(9):
                ...
                # 使用Frame实现边框效果
                self.entry_frame = ttk.Frame(grid_frame, borderwidth=0, relief=tk.FLAT)
                self.entry_frame.grid(row=i, column=j, padx=padx, pady=pady)
                entry.grid(in_=self.entry_frame, ipadx=6, ipady=6)
                entry.lift()  # 将输入框置于顶层
                
                # 设置背景色区分九宫格
                if (i // 3 + j // 3) % 2 == 0:
                    entry.configure(background="#e8f4f8", foreground="#000000")
                    self.entry_frame.configure(style="Odd.TFrame")
                else:
                    entry.configure(background="#ffffff", foreground="#000000")
                    self.entry_frame.configure(style="Even.TFrame")
        ...
        # 求解时间显示标签 - 单位改为毫秒
        self.time_label = ttk.Label(right_control, text="求解耗时: -- ms", style="Time.TLabel")
        ...
        # 检查按钮
        check_button = ttk.Button(right_control, text="检查", command=self.check_board)
    ...
    def solve_sudoku(self):
        ...
        # 计算毫秒耗时并格式化显示
        solve_duration_ms = (end_time - start_time) * 1000  # 转换为毫秒
        self.time_label.config(text=f"求解耗时: {solve_duration_ms:.3f} ms")  # 保留3位小数
    ...
    def clear_board(self):
        ...
        self.time_label.config(text="求解耗时: -- ms")  # 更新为毫秒单位
    ...
    def check_board(self):
            ...
            for box_j in range(0, 9, 3):
                ...
                if len(box_values) != len(set(box_values)):
                    ...
                    for i in range(box_i, box_i + 3):
                        for j in range(box_j, box_j + 3):
                            if board[i][j] != 0:
                                box_conflicts.add((i, j))
        
        # 合并所有冲突单元格
        all_conflicts = row_conflicts | col_conflicts | box_conflicts
        ...
        # 显示检查结果
        if all_conflicts:
            messagebox.showwarning("检查结果", f"发现{len(all_conflicts)}个冲突！")
        else:
    ...
    def check_cell(self, row, col):
        ...
        # 检查当前单元格是否有冲突
        if not solver.is_valid(row, col, num):
    ...
    def reset_cell_style(self, row, col):
        ...
        # 恢复输入框背景
        if (row // 3 + col // 3) % 2 == 0:
            self.entries[row][col].configure(background="#e8f4f8")
        else:
            self.entries[row][col].configure(background="#ffffff")
        
        # 恢复Frame背景
        frame = self.entries[row][col].master
        if (row // 3 + col // 3) % 2 == 0:
            frame.configure(style="Odd.TFrame")
        else:
