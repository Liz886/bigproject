        ...
        try:
            ...
            for contour in contours:
                ...
                if len(approx) == 4:
                    ...
                    if 0.9 <= aspect_ratio <= 1.1:  # 允许一定的宽高比误差
            ...
            self.update_progress(60, "正在识别数字...")
            # 分割并识别每个单元格
            for i in range(9):
                for j in range(9):
                    # 计算单元格坐标
                    ...
                    # 查找数字轮廓
                    cell_contours, _ = cv2.findContours(cell.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                    
                    if cell_contours:
                        # 找到最大轮廓（数字）
                        c = max(cell_contours, key=cv2.contourArea)
                        area = cv2.contourArea(c)
                        
                        # 过滤小轮廓（可能是噪声）
                        if area > 10:
                            # 提取ROI并调整大小
                            roi = cv2.resize(cell, (30, 30))
                            
                            # 使用Tesseract识别数字
                            try:
                                text = pytesseract.image_to_string(roi, config='--psm 8 --oem 3 -c tessedit_char_whitelist=123456789')
                                digit = text.strip()
                                
                                if digit and digit in '123456789':
                                    self.board[i][j] = int(digit)
                                else:
                                    empty_cells += 1
                            except Exception as e:
                                empty_cells += 1
                                self.log_error(f"数字识别失败: {str(e)}")
                        else:
                            empty_cells += 1
                    else:
                        empty_cells += 1
            ...
            self.update_progress(45, "正在分割数独网格...")
            # 分割单元格并识别数字
            board = [[0 for _ in range(9)] for _ in range(9)]
            cell_size = maxWidth // 9
            ...
            for i in range(9):
                for j in range(9):
                    # 更新进度
                    self.recognition_progress = 45 + (i * 9 + j) * progress_per_cell
